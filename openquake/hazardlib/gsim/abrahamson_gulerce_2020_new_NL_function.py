...
# Path to the new NL model coefficients
NL_FUNCTION_COEFFS_FFP = os.path.join(os.path.dirname(__file__), "NL_model_inputs_proposed_adjustments_FINAL_v3.csv")
NL_FUNCTION_COEFFS = pd.read_csv(NL_FUNCTION_COEFFS_FFP)

# specify which NL model version to use ('mild', 'moderate', 'aggressive')
# nl_model_type = 'mild'
# nl_model_type = 'moderate'
nl_model_type = 'aggressive'
...
def get_coeffs_for_new_NL_function(ctx, NL_model_type):

    f2_array = np.array([ NL_FUNCTION_COEFFS[ (NL_FUNCTION_COEFFS['Vs30'] == vs30_val ) &\
                                   (NL_FUNCTION_COEFFS['model_id'] == NL_model_type )]['f2'].values[0] for vs30_val in ctx.vs30])

    f3_array = np.array([NL_FUNCTION_COEFFS[(NL_FUNCTION_COEFFS['Vs30'] == vs30_val) & \
                                            (NL_FUNCTION_COEFFS['model_id'] == NL_model_type)]['f3'].values[0] for vs30_val in
                         ctx.vs30])
    n_array = np.array([NL_FUNCTION_COEFFS[(NL_FUNCTION_COEFFS['Vs30'] == vs30_val) & \
                                            (NL_FUNCTION_COEFFS['model_id'] == NL_model_type)]['n'].values[0] for vs30_val in
                         ctx.vs30])

    return f2_array, f3_array, n_array
...
def _generic_NL_model_single_corner_with_n(f_2, f_3, pga_rock_g, n):
    '''
    @param f_2: float: slope of the nonlinear function
    @param f_3: float: value for the transition between linear and nonlinear site amp (in g)
    @param pga_rock_g: numpy array: peak ground acceleration on 760 m/s in g
    @param n: float: n-exponent of nonlinear function
    @return: numpy array: nonlinear site response in natural log units
    '''

    fnl = f_2 * np.log((pga_rock_g / f_3)**n + 1)

    return fnl

def get_site_amplification_term_new_NL_function(C, region, vs30, pga760, f2, f3, n_exp):
    """
    Returns the shallow site amplification term as descrbied in Equation 3.7,
    and corrected in the Erratum
    ...
    :param numpy.ndarray vs30:
        30-m averaged shearwave velocity (m/s)
    :param numpy.ndarray pga760:
        Peak Ground Acceleration (PGA), g, on a reference bedrock of 760 m/s
    """
    a12 = C["a12"]
    if region != "GLO":
        # Apply regional adjustment
    ...
    # Linear site term
    fsite = a12 * np.log(vnorm) + (C["b"] * CONSTS["n"] * np.log(vnorm))
    idx = vstar >= C["vlin"]

    idx = np.logical_not(idx)
    # Nonlinear site term
    fsite[idx] = fsite[idx] + _generic_NL_model_single_corner_with_n(f2, f3, pga760[idx], n_exp)
...
def get_acceleration_on_760mps(C, trt, region, ctx, apply_adjustment, pga1000):
    """
    Returns acceleration on Vs30 = 760 m/s - intended for use primarily with
    PGA. Overrides the Vs30 values and removes any basin depth terms
    Uses the original site amp model to get from pga1000 to pga760
    """
    # Set all Vs30 to 760 m/s
    vs30 = np.full_like(ctx.vs30, 760.)

    # No basin depth is calculated here (not using CAS or JPN regions)
    return (get_base_term(C, region, apply_adjustment) +
            get_magnitude_scaling_term(C, trt, region, ctx.mag) +
            get_geometric_spreading_term(C, region, ctx.mag, ctx.rrup) +
            get_anelastic_attenuation_term(C, region, ctx.rrup) +
            get_rupture_depth_scaling_term(C, trt, ctx) +
            get_inslab_scaling_term(C, trt, region, ctx.mag, ctx.rrup) +
            get_site_amplification_term(C, region, vs30, pga1000) +
            get_backarc_term (trt, PGA(), ctx))


def get_mean_acceleration(C, trt, region, ctx, pga760, apply_adjustment, f2, f3, n_exp):
    """
    Returns the mean acceleration on soil using the new NL function
    """
    ...
    return (get_base_term(C, region, apply_adjustment) +
            ...
            get_site_amplification_term_new_NL_function(C, region, ctx.vs30, pga760, f2, f3, n_exp) +
...
def get_partial_derivative_site_pga_new_NL_function(f_2, f_3, pga_rock, n):

    '''
    compute the partial derivative of the new NL function, used to calculate Nl std devs

    @param f_2: slope of NL function
    @param f_3: Transition value from linear to NL site amp (units of g)
    @param pga_rock: np.array of PGA reference condition (760 m/s for new NL function). Units of g
    @param n: exponent for NL function
    @return:
    '''

    dFnl_over_dlnPGA = f_2 * n / (1 + (f_3 / pga_rock) ** n)

    return dFnl_over_dlnPGA

def get_tau_phi(C, C_PGA, region, period, rrup, vs30, pga760, ergodic, f_2, f_3, n_exp):
    """
    Get the heteroskedastic within-event and between-event standard deviation
    Uses the inputs for the new proposed NL function
    """
    ...
    partial_f_pga = get_partial_derivative_site_pga_new_NL_function(f_2, f_3, pga760[idx], n_exp)
...
class AbrahamsonGulerce2020SInter(GMPE):
    ...
    def compute(self, ctx: np.recarray, imts, mean, sig, tau, phi):
        ...
        pga760 = np.exp( get_acceleration_on_760mps(C_PGA, trt,
                                                     self.region, ctx,
                                                     self.apply_usa_adjustment, pga1000) )

        # get the inputs for NL function based on vs30 values and nl_model_type which is hard-coded above
        f2_arr, f3_arr, n_arr = get_coeffs_for_new_NL_function(ctx, nl_model_type)

        for m, imt in enumerate(imts):
            C = self.COEFFS[imt]
            # mean[m] = get_mean_acceleration(C, trt, self.region, ctx, pga1000,
            #                                 self.apply_usa_adjustment) + get_backarc_term(trt, imt, ctx)
            # get the mean using the new NL function inputs
            # mean[m] = get_mean_acceleration(C, trt, self.region, ctx, pga760,
            #                                 self.apply_usa_adjustment, CONSTS_NL['f2'], CONSTS_NL['f3'], CONSTS_NL['n'])\
            #                                 + get_backarc_term(trt, imt, ctx)
            mean[m] = get_mean_acceleration(C, trt, self.region, ctx, pga760,
                                            self.apply_usa_adjustment, f2_arr, f3_arr, n_arr) \
                      + get_backarc_term(trt, imt, ctx)
            ...
            # Get the standard deviations using the new NL function inputs
            # tau_m, phi_m = get_tau_phi(C, C_PGA, self.region, imt.period,
            #                            ctx.rrup, ctx.vs30, pga760,
            #                            self.ergodic, CONSTS_NL['f2'], CONSTS_NL['f3'], CONSTS_NL['n'])
            tau_m, phi_m = get_tau_phi(C, C_PGA, self.region, imt.period,
                                       ctx.rrup, ctx.vs30, pga760,
                                       self.ergodic, f2_arr, f3_arr, n_arr)

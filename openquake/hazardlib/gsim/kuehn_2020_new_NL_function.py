...
def _generic_NL_model_single_corner_with_n(f_2, f_3, pga_rock_g, n):
    ...
    return fnl
def get_shallow_site_response_term_new_NL_function(C, region, vs30, pga760, f2, f3, n_exp):
    """
    ...
    :param numpy ndarray pga760:
        Peak ground acceleration on reference rock for the new NL function (Vs30 760 m/s) in g
    """
    # c7 is the same for interface or inslab - so just read from interface
    c_7 = C[REGION_TERMS_IF[region]["c7"]]
    # Get linear site term (original from kuehn_2020)
    vs_mod = vs30 / C["k1"]
    f_site_g = c_7 * np.log(vs_mod) + (C["k2"] * CONSTS["n"] *
                                     np.log(vs_mod))
    idx = vs30 > C["k1"]
    # f_site_g[idx] = f_site_g[idx] + (C["k2"] * CONSTS["n"] *
    #                                  np.log(vs_mod[idx]))

    # Get nonlinear site response term (proposed modified NL function)
    idx = np.logical_not(idx)
    if np.any(idx):
        f_site_g[idx] = f_site_g[idx] + _generic_NL_model_single_corner_with_n(f2, f3, pga760[idx], n_exp)
...
def get_nonlinear_stddevs(C, C_PGA, imt, vs30, pga760, f_2, f_3, n_exp):
    """
...
def get_mean_values_760mps(C, region, trt, m_b, ctx, pga1100):
    """
    Returns the mean ground values for a Vs30 value of 760 m/s for use with nonlinear function

    :param float m_b:
        Magnitude scaling breakpoint
    """
    # Define a Vs30 of 760 m/s
    vs30 = 760.0 * np.ones(ctx.vs30.shape)
    ...
    mean = (get_base_term(C, trt, region) +
            ...
            get_anelastic_attenuation_term(C, trt, region, ctx.rrup) +
            get_depth_term(C, trt, ctx.ztor) +
            get_shallow_site_response_term(C, region, vs30, pga1100))

    return mean

def get_mean_values_new_NL_function(C, region, trt, m_b, ctx, pga760, f2, f3, n_exp):
    """
    Returns the mean ground values for a specific IMT
    ...
    mean = (get_base_term(C, trt, region) +
            ...
            get_anelastic_attenuation_term(C, trt, region, ctx.rrup) +
            get_depth_term(C, trt, ctx.ztor) +
            get_shallow_site_response_term_new_NL_function(C, region, ctx.vs30, pga760, f2, f3, n_exp))
            # get_shallow_site_response_term(C, region, ctx.vs30, pga760))
...
class KuehnEtAl2020SInter(GMPE):
    ...
    def compute(self, ctx: np.recarray, imts, mean, sig, tau, phi):
        ...
        # Get PGA on 760 m/s for use with NL function
        pga760 = np.exp(get_mean_values_760mps(C_PGA, self.region, trt, m_b,
        ...
        for imt in imts:
            if ("PGA" in imt.string) or (("SA" in imt.string) and
                                         (imt.period <= 0.1)):
                pga_soil = get_mean_values_new_NL_function(C_PGA, self.region, trt, m_b,
                                           ctx, pga760, f2_arr, f3_arr, n_arr) + get_backarc_term(trt, PGA(), ctx)
                break
        ...
        for m, imt in enumerate(imts):
            ...
            if self.which_sigma == "Modified":
                sig[m], tau[m], phi[m] = get_nonlinear_stddevs(C, C_PGA, imt, ctx.vs30, pga760, f2_arr, f3_arr, n_arr)
            else:
                tau[m] = C["tau"]
                phi[m] = C["phi"]
